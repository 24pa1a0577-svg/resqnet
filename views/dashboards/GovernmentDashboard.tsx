...
const GovernmentDashboard: React.FC<{ user: User, onLogout: () => void }> = ({ user, onLogout }) => {
  ...
  const handleUpdateRequestStatus = (requestId: string, newStatus: 'Approved' | 'Rejected') => {
    ...
    setRequests(updated);
    storage.set(REQUESTS_KEY, updated);
  };
        ...
        <div className="max-w-5xl mx-auto space-y-6">
                  ...
                  {r.status === 'Pending' ? (
                    <>
                      <button 
                        ...
                        className="px-6 py-3 bg-green-600 text-white font-bold rounded-2xl shadow-lg shadow-green-100 hover:bg-green-700 transition"
                      >
                        Approve
                      </button>
                      <button 
                    ...
                    <span className="px-6 py-3 bg-green-50 text-green-600 font-bold rounded-2xl flex items-center gap-2">
                      <i className="fas fa-check-circle"></i> Fulfilled Request
                    </span>
                  ) : (
                    <span className="px-6 py-3 bg-red-50 text-red-600 font-bold rounded-2xl flex items-center gap-2">
           ...
           {requests.length === 0 && (
             <div className="text-center py-20 bg-white rounded-3xl border border-gray-100 text-gray-400">
                No active resource requests found.
             </div>
           )}
        </div>
      )}
